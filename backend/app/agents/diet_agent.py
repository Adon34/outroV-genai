# backend/app/agents/diet_agent.py
import logging
...
from app.agents.prompts import DIET_AGENT_PROMPT

logger = logging.getLogger(__name__)


class ModernDietAgent(BaseAgent):
    """
    Agente especializado em criar planos alimentares personalizados.
    ...
    async def process(self, message: str, user_id: int, db=None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Erro no DietAgent: {e}", exc_info=True)
            return self._format_response(
                message="Desculpe, tive um problema ao preparar sua dieta. Pode tentar novamente?",
